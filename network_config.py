...
class NetworkInfo:
    ...
    host_ip_source: str  # "env" | "auto" | "unavailable"
    stream_key: str
    rtmp_port: int
    rtsp_port: int
    webrtc_port: int
...
def get_mediamtx_status(info: Optional[NetworkInfo] = None, timeout: float = 0.3) -> dict:
    ...
    Uses a short timeout by default since this is meant to be polled
    frequently (e.g. from a dashboard refresh) and must stay responsive
