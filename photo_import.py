...
This is the automatic bridge between "the DJI Neo 2 operator pressed the
shutter" and "MatanglaWIN analyzed the photo" - with NO manual upload and
NO Analyze button in between. The application watches a local folder; when
a new image file appears and has finished copying, it is analyzed exactly
once through the SAME central pipeline as a manual upload
(inspection_service.InspectionService), and the website updates itself.

How a DJI photo reaches that folder (the real, verified transfer path):
the DJI Neo 2 stores the still on the drone/goggles/RC; the DJI Fly app
(or a phone auto-sync / the DJI transfer tool) copies new captures to the
PC. Point that destination - or a folder synced to it - at this watch
folder (MATANGLAWIN_IMPORT_DIR) and every captured photo flows in
automatically. This module never claims to talk to the drone directly; it
reliably reacts to files appearing on disk, which is what makes the
"take photo -> automatic transfer -> automatic analysis -> automatic
result" experience work regardless of the exact transfer tool.
...
never affects inspection of imported (or uploaded) photos, and vice versa.
"""
